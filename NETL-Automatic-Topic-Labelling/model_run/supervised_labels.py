...
parser.add_argument(
    "svm_classify")  # path to the SVM Ranker classify binary file. Needs to be downloaded from the path provided in Readme.
...
def get_lt_ranks(lab_list, num):
    ...
    for item in lab_list:
        ...
        for key in label_cnt:
            label_cnt[key] /= total
        tot_keys = list(set(topic_ls + label_cnt))
        listtopic = []
        listlabel = []
...
def prepare_features(letter_tg_dict, page_rank_dict, cols, feature_names):
    ...
    for x in range(0, len(letter_tg_dict)):
        ...
        for k in a:
            ...
            new_list.append(
                3)  # This could be just any value appended for the sake of giving a column for annotator rating neeeded in SVM Ranker classify
